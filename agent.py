...
class Agent_Attack():
    ...
    def reward(self, img, prob, orig_img):
        new_prob = self.classifier_img(img)
        img_diff = torch.norm(img - orig_img)
        prob_diff = torch.norm(prob - new_prob)
        # return 2*prob_diff - img_diff
        return torch.log(prob_diff + 1e-5) - torch.log(img_diff + 1e-5)
    ...
    def optimize_model(self):
        ...
        next_state_batch = torch.stack(batch.next_state).cuda()
    ...
    def train(self):
        ...
        for img, label in tqdm(self.train_loader):
            ...
            self.optimizer.zero_grad()
            state = torch.cat([img.view(-1), torch.tensor([x, y, r]), torch.tensor(self.history).float(), orig_prob.squeeze(), orig_prob.squeeze()])
            for j in range(self.max_iter):
                ...
                reward = self.reward(img, orig_prob, orig_img)
                self.reward_lists.append(reward.item())
                self.history.append(action)
                next_state = torch.cat([img.view(-1).cpu(), torch.tensor([x, y, r]), torch.tensor(self.history).float(), prob.squeeze(), orig_prob.squeeze()])
