...
def _mnist(split: str) -> Dataset:
    dataset_path = os.path.join(os.getenv('PT_DATA_DIR', 'datasets'), 'dataset_cache')
    if split == "train":
        return datasets.MNIST(dataset_path, train=True, download=True, transform=transform_mnist)
    elif split == "test":
        return datasets.MNIST(dataset_path, train=False, download=True, transform=transform_mnist)
    else:
        raise Exception("Unknown split name.")
